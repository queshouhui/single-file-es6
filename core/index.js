...
var _processorHelper = require("./processor-helper.js");
/*
...
class SingleFileClass {
  constructor(options) {
    ...
    const ProcessorHelper = (0, _processorHelper.getProcessorHelperClass)(options, util);
...
const STAGES = [{
  ...
  sequential: [{
    ...
    option: "saveFilenameTemplateData",
    action: "saveFilenameTemplateData"
  }]
}];
class Runner {
  ...
  async loadPage() {
    await this.onprogress(new ProgressEvent(PAGE_LOADING, {
      pageURL: this.options.url,
      frame: !this.root,
      options: this.options
    }));
    await this.processor.loadPage(this.options.content);
    await this.onprogress(new ProgressEvent(PAGE_LOADED, {
      pageURL: this.options.url,
      frame: !this.root,
      options: this.options
    }));
  ...
  async initialize() {
    await this.onprogress(new ProgressEvent(RESOURCES_INITIALIZING, {
      pageURL: this.options.url,
      options: this.options
    }));
  ...
  async run() {
    if (this.root) {
      ...
      await this.onprogress(new ProgressEvent(RESOURCES_INITIALIZED, {
        pageURL: this.options.url,
  ...
  async getPageData() {
    if (this.root) {
      await this.onprogress(new ProgressEvent(PAGE_ENDED, {
        pageURL: this.options.url,
        options: this.options
      }));
    }
  ...
  async executeStage(step) {
    ...
    await this.onprogress(new ProgressEvent(STAGE_STARTED, {
      pageURL: this.options.url,
      step,
      frame,
      options: this.options
    ...
    await this.onprogress(new ProgressEvent(STAGE_ENDED, {
      pageURL: this.options.url,
      step,
      frame,
      options: this.options
...
class Processor {
  ...
  removeDiscardedResources() {
    ...
    replacedAttributeValue.forEach(element => {
      const relValue = element.getAttribute("rel").replace(/(preconnect|prerender|dns-prefetch|preload|prefetch|manifest|modulepreload)/g, "").trim();
      if (relValue.length) {
  ...
  resolveStyleAttributeURLs() {
    this.doc.querySelectorAll("[style]").forEach(element => {
      if (this.options.blockStylesheets) {
        ...
        const declarationList = _processorHelper.cssTree.parse(styleContent, {
  ...
  removeAlternativeMedias() {
    const stats = util.minifyMedias(this.stylesheets, {
      keepPrintStyleSheets: this.options.keepPrintStyleSheets
    });
  ...
  async processStyleAttributes() {
    return Promise.all([...this.styles].map(([, stylesheet]) => this.processorHelper.processStyle(stylesheet, this.options, this.resources, this.batchRequest)));
  }
  ...
  async processWorklets() {
    if (this.options.worklets.length) {
      ...
      await Promise.all(this.options.worklets.map(async ({
        moduleURL,
        options
      }) => {
  ...
  compressHTML() {
    ...
    util.minifyHTML(this.doc, {
      PRESERVED_SPACE_ELEMENT_ATTRIBUTE_NAME: util.PRESERVED_SPACE_ELEMENT_ATTRIBUTE_NAME
    });
    if (this.options.displayStats) {
  ...
  async setDocInfo() {
    ...
    this.options.infobarContent = await util.evalTemplate(this.options.infobarTemplate, this.options, null, this.doc, {
      dontReplaceSlash: true
    });
  }
}
