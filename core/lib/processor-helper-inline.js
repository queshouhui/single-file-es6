...
function getProcessorHelperClass(utilInstance) {
  ...
  return class ProcessorHelper extends ProcessorHelperCommon {
      ...
      await Promise.all(imports.map(async node => {
        ...
        if (urlNode) {
          ...
          if (!(0, _processorHelperCommon.testIgnoredPath)(resourceURL) && (0, _processorHelperCommon.testValidPath)(resourceURL)) {
            ...
            if ((0, _processorHelperCommon.testValidURL)(resourceURL) && !importedStyleSheets.has(resourceURL)) {
              options.inline = true;
          ...
          if (!(0, _processorHelperCommon.testIgnoredPath)(resourceURL) && (0, _processorHelperCommon.testValidURL)(resourceURL)) {
            ...
            if (!originalResourceURL.startsWith("#")) {
              ...
              if (duplicate && options.groupDuplicateImages && util.getContentSize(content) < maxSizeDuplicateImages) {
                const varNode = cssTree.parse("var(" + SINGLE_FILE_VARIABLE_NAME_PREFIX + indexResource + ")", {
                  context: "value"
                });
                for (let keyName of Object.keys(varNode.children.head.data)) {
                  urlNode[keyName] = varNode.children.head.data[keyName];
                }
                resources.cssVariables.set(indexResource, {
                  content,
                      ...
                      if (!forbiddenPrefixFound) {
                        ...
                        if (processDuplicates && duplicate && !isSVG && util.getContentSize(content) < maxSizeDuplicateImages) {
                          if (this.replaceImageSource(resourceElement, SINGLE_FILE_VARIABLE_NAME_PREFIX + indexResource, options)) {
                            ...
                            const declarationList = cssTree.parse(resourceElement.getAttribute("style"), {
    ...
    async processScript(element, resourceURL, options, charset, batchRequest) {
      let content = (0, _processorHelperCommon.getUpdatedResourceContent)(resourceURL, options);
      if (content) {
        content = await (0, _processorHelperCommon.toDataURI)(content, "text/javascript", charset);
      } else {
    ...
    async processFontFaceRule(ruleData, fontInfo, fontDeclarations, fontTests, stats) {
      ...
      if (srcDeclaration) {
        ...
        const woffFontFound = findSourceByFormat("woff2-variations", true) || findSourceByFormat("woff2", true) || findSourceByFormat("woff", true);
        if (woffFontFound) {
          fontInfo = filterSources(woffFontFound);
        } else {
          const ttfFontFound = findSourceByFormat("truetype-variations", true) || findSourceByFormat("truetype", true);
          if (ttfFontFound) {
            fontInfo = filterSources(ttfFontFound);
          } else {
            const otfFontFound = findSourceByFormat("opentype") || findSourceByFormat("embedded-opentype");
            if (otfFontFound) {
