...
exports.getImportFunctions = getImportFunctions;
exports.getProcessorHelperCommonClass = getProcessorHelperCommonClass;
exports.getUpdatedResourceContent = getUpdatedResourceContent;
exports.getUrlFunctions = getUrlFunctions;
exports.isDataURL = isDataURL;
exports.matchCharsetEquals = matchCharsetEquals;
exports.normalizeURL = normalizeURL;
exports.replaceOriginalURLs = replaceOriginalURLs;
exports.testIgnoredPath = testIgnoredPath;
exports.testValidPath = testValidPath;
exports.testValidURL = testValidURL;
...
class ProcessorHelperCommon {
  async processPageResources(doc, baseURI, options, resources, styles, batchRequest) {
    const processAttributeArgs = [["link[href][rel*=\"icon\"]", "href", true], ["object[type=\"image/svg+xml\"], object[type=\"image/svg-xml\"], object[data*=\".svg\"]", "data"], ["img[src], input[src][type=image]", "src", false, true], ["embed[src*=\".svg\"]", "src"], ["video[poster]", "poster"], ["*[background]", "background"], ["image", "xlink:href"], ["image", "href"]];
    if (options.blockImages) {
      doc.querySelectorAll("svg").forEach(element => element.remove());
    }
    let resourcePromises = processAttributeArgs.map(([selector, attributeName, removeElementIfMissing, processDuplicates]) => this.processAttribute(doc.querySelectorAll(selector), attributeName, baseURI, options, "image", resources, removeElementIfMissing, batchRequest, styles, processDuplicates));
    resourcePromises = resourcePromises.concat([this.processXLinks(doc.querySelectorAll("use"), doc, baseURI, options, batchRequest), this.processSrcset(doc.querySelectorAll("img[srcset], source[srcset]"), baseURI, options, resources, batchRequest)]);
    ...
    if (removedSelectors.length) {
      removedSelectors.forEach(({
        ...
        if (parentSelector.data.children.size == 0 || !selector.prev || selector.prev.data.type == "Combinator" || selector.prev.data.type == "WhiteSpace") {
          parentSelector.data.children.replace(selector, cssTree.parse("*", {
            context: "selector"
          }).children.head);
        } else {
  ...
  getFontKey(ruleData) {
    return JSON.stringify([(0, _helper.normalizeFontFamily)(this.getPropertyValue(ruleData, "font-family")), (0, _helper.getFontWeight)(this.getPropertyValue(ruleData, "font-weight") || "400"), this.getPropertyValue(ruleData, "font-style") || "normal", this.getPropertyValue(ruleData, "unicode-range"), getFontStretch(this.getPropertyValue(ruleData, "font-stretch")), this.getPropertyValue(ruleData, "font-variant") || "normal", this.getPropertyValue(ruleData, "font-feature-settings"), this.getPropertyValue(ruleData, "font-variation-settings")]);
  }
    ...
    fontsDetails.fonts.set(fontKey, fontInfo.map(fontSource => {
      ...
      if (fontResources) {
        ...
        return {
          ...
          fontUrl,
          format: fontFormat,
          contentType: fontResource && fontResource.contentType
        };
      } else {
        return {
          src: fontSource.match(REGEXP_FONT_SRC)[1],
...
function getURL(urlFunction) {
  ...
  const urlMatch = urlFunction.match(REGEXP_URL_SIMPLE_QUOTES_FN) || urlFunction.match(REGEXP_URL_DOUBLE_QUOTES_FN) || urlFunction.match(REGEXP_URL_NO_QUOTES_FN);
