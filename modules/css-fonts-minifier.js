...
var fontPropertyParser = _interopRequireWildcard(require("./../vendor/css-font-property-parser.js"));
...
const helper = {
  normalizeFontFamily: _helper.normalizeFontFamily,
  flatten: _helper.flatten,
  getFontWeight: _helper.getFontWeight,
  removeQuotes: _helper.removeQuotes
};
    ...
    if (ruleData.type == "Atrule" && (ruleData.name == "media" || ruleData.name == "supports" || ruleData.name == "layer" || ruleData.name == "container") && ruleData.block && ruleData.block.children) {
      ...
      if (ruleData.type == "Atrule" && ruleData.name == "font-face") {
        ...
        if (fontFamily) {
          ...
          fontWeight.split(",").forEach(weightValue => fontsInfo.declared.push({
            fontFamily,
            fontWeight: helper.getFontWeight(helper.removeQuotes(weightValue)),
            fontStyle,
            fontVariant
          }));
        }
...
function testUsedFont(ruleData, familyName, declaredFonts, filteredUsedFonts) {
  ...
  if (optionalUsedFonts && optionalUsedFonts.length) {
    ...
    if (VALID_FONT_STYLES.find(rule => fontStyle.trim().match(rule))) {
      ...
      const declaredFontsWeights = declaredFonts.filter(fontInfo => fontInfo.fontFamily == familyName && fontInfo.fontStyle == fontStyle).map(fontInfo => fontInfo.fontWeight.split(" ")).sort((weight1, weight2) => Number.parseInt(weight1[0], 10) - Number.parseInt(weight2[0], 10));
      let usedFontWeights = optionalUsedFonts.map(fontInfo => getUsedFontWeight(fontInfo, fontStyle, declaredFontsWeights)).filter(fontWeight => fontWeight);
...
function testFontweight(fontWeight, usedFontWeights) {
  ...
  for (const fontWeightValue of fontWeight.split(",")) {
    let {
      min: fontWeightMin,
      max: fontWeightMax
    } = parseFontWeight(fontWeightValue);
    if (!fontWeightMax) {
      fontWeightMax = 900;
    }
    test = test || usedFontWeights.find(usedFontWeight => {
      let {
        min: usedFontWeightMin,
        max: usedFontWeightMax
      } = parseFontWeight(usedFontWeight);
...
function getFontFamilyNames(declarations, options) {
  ...
  if (font && font.data && font.data.value) {
    try {
      ...
      if (matchedVar && matchedVar[1]) {
        value = cssTree.parse(globalThis.getComputedStyle(options.doc.body).getPropertyValue(matchedVar[1]), {
          context: "value"
        });
      }
