...
var _templateParser = require("./template-parser.js");
...
async function formatFilename(content, doc, options) {
  ...
  if (!options.keepFilename && (options.filenameMaxLengthUnit == "bytes" && (0, _helper.getContentSize)(filename) > options.filenameMaxLength || filename.length > options.filenameMaxLength)) {
...
async function evalTemplate(template = "", options, content, doc, context = {}) {
  ...
  const variables = {
    ...
    "page-title": {
      getter: () => options.title
    },
    "page-heading": {
      getter: () => options.info.heading
    },
    "page-language": {
      getter: () => options.info.lang
    },
    "page-description": {
      getter: () => options.info.description
    },
    "page-author": {
      getter: () => options.info.author
    },
    "page-creator": {
      getter: () => options.info.creator
    },
    "page-publisher": {
      getter: () => options.info.publisher
    },
    "url-hash": {
      getter: () => url.hash.substring(1)
    },
    "url-host": {
      getter: () => url.host.replace(/\/$/, "")
    },
    "url-hostname": {
      getter: () => url.hostname.replace(/\/$/, "")
    },
    "url-hostname-suffix": {
      getter: () => urlSuffix
    },
    ...
    "url-hostname-subdomains": {
      getter: () => urlSubDomains
    },
    "url-href": {
      getter: () => urlHref,
    ...
    "url-href-digest-sha-1": {
      getter: urlHref ? async () => (0, _helper.digest)("SHA-1", urlHref) : ""
    },
    "url-href-flat": {
      getter: () => decode(url.href),
      dontReplaceSlash: false
    },
    "url-referrer": {
      getter: () => decode(options.referrer),
      dontReplaceSlash: dontReplaceSlashIfUndefined
    },
    "url-referrer-flat": {
      getter: () => decode(options.referrer),
    ...
    "url-pathname": {
      getter: () => decode(url.pathname).replace(/^\//, "").replace(/\/$/, ""),
      dontReplaceSlash: dontReplaceSlashIfUndefined
    },
    "url-pathname-flat": {
      getter: () => decode(url.pathname),
      dontReplaceSlash: false
    },
    "url-port": {
      getter: () => url.port
    ...
    "url-search": {
      getter: () => url.search.substring(1)
    },
    "url-username": {
      getter: () => url.username
    },
    "tab-id": {
      getter: () => String(options.tabId)
    },
    "tab-index": {
      getter: () => String(options.tabIndex)
    },
    "url-last-segment": {
      getter: () => decode(getLastSegment(url, options.filenameReplacementCharacter))
    },
    "bookmark-pathname": {
      getter: () => bookmarkFolder,
      dontReplaceSlash: dontReplaceSlashIfUndefined
    },
    ...
    "profile-name": {
      getter: () => options.profileName
    },
    "filename-extension": {
      getter: () => getFilenameExtension(options)
    },
    "save-action": {
      getter: () => options.selected ? "selection" : "page"
    },
    "options-json": {
      getter: () => JSON.stringify(getFilteredOptions(options))
    },
    "options-text": {
      getter: () => optionsToText(getFilteredOptions(options))
    }
  };
  if (content) {
    variables["digest-sha-256"] = {
      getter: async () => (0, _helper.digest)("SHA-256", content)
    };
    variables["digest-sha-384"] = {
      getter: async () => (0, _helper.digest)("SHA-384", content)
    };
    variables["digest-sha-512"] = {
      getter: async () => (0, _helper.digest)("SHA-512", content)
    };
  }
  if (options.saveDate) {
  ...
  template = replaceAll(template, "\\|", "\\\\|");
  template = replaceAll(template, "\\>", "\\\\>");
  let result = await (0, _templateParser.parse)(template, {
    async callFunction(name, [argument, optionalArguments], lengthData) {
      ...
      if (fn) {
        ...
        optionalArguments = optionalArguments.map(argument => argument.replace(/\\\\(.)/g, "$1")).filter(argument => argument != undefined && argument != null && argument != "");
  ...
  function addDateVariables(date, prefix = "") {
    variables[prefix + "datetime-iso"] = {
      getter: () => date.toISOString()
    };
    variables[prefix + "date-iso"] = {
      getter: () => date.toISOString().split("T")[0]
    };
    variables[prefix + "time-iso"] = {
      getter: () => date.toISOString().split("T")[1].split("Z")[0]
    };
    variables[prefix + "date-locale"] = {
      getter: () => date.toLocaleDateString()
    };
    variables[prefix + "time-locale"] = {
      getter: () => date.toLocaleTimeString()
    };
    variables[prefix + "day-locale"] = {
      getter: () => String(date.getDate()).padStart(2, "0")
    };
    variables[prefix + "month-locale"] = {
      getter: () => String(date.getMonth() + 1).padStart(2, "0")
    };
    variables[prefix + "year-locale"] = {
      getter: () => String(date.getFullYear())
    };
    variables[prefix + "datetime-locale"] = {
      getter: () => date.toLocaleString()
    };
    variables[prefix + "datetime-utc"] = {
      getter: () => date.toUTCString()
    };
    variables[prefix + "day-utc"] = {
      getter: () => String(date.getUTCDate()).padStart(2, "0")
    };
    variables[prefix + "month-utc"] = {
      getter: () => String(date.getUTCMonth() + 1).padStart(2, "0")
    };
    variables[prefix + "year-utc"] = {
      getter: () => String(date.getUTCFullYear())
    };
    variables[prefix + "hours-locale"] = {
      getter: () => String(date.getHours()).padStart(2, "0")
    };
    variables[prefix + "minutes-locale"] = {
      getter: () => String(date.getMinutes()).padStart(2, "0")
    };
    variables[prefix + "seconds-locale"] = {
      getter: () => String(date.getSeconds()).padStart(2, "0")
    };
    variables[prefix + "hours-utc"] = {
      getter: () => String(date.getUTCHours()).padStart(2, "0")
    };
    variables[prefix + "minutes-utc"] = {
      getter: () => String(date.getUTCMinutes()).padStart(2, "0")
    };
    variables[prefix + "seconds-utc"] = {
      getter: () => String(date.getUTCSeconds()).padStart(2, "0")
    };
    variables[prefix + "time-ms"] = {
      getter: () => String(date.getTime())
    };
  }
}
...
function getValidFilename(filename, replacedCharacters = DEFAULT_REPLACED_CHARACTERS, replacementCharacter = DEFAULT_REPLACEMENT_CHARACTER) {
  ...
  filename = filename.replace(/\.\.\//g, "").replace(/^\/+/, "").replace(/\/+/g, "/").replace(/\/$/, "").replace(/\.$/, "").replace(/\.\//g, "." + replacementCharacter).replace(/\/\./g, "/" + replacementCharacter);
  return filename;
}
  ...
  return new Promise((resolve, reject) => {
    reader.addEventListener("load", () => {
      if (content.startsWith(reader.result)) {
        ...
        truncateText(content, maxSize - 1).then(resolve).catch(reject);
      }
    }, false);
