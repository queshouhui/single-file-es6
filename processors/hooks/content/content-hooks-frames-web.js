...
(globalThis => {
  ...
  function init() {
    ...
    document.addEventListener(DISPATCH_SCROLL_START_EVENT, () => {
      dispatchScrollEvent = true;
    });
    document.addEventListener(DISPATCH_SCROLL_END_EVENT, () => {
    ...
    document.addEventListener(BLOCK_COOKIES_END_EVENT, () => {
    ...
    document.addEventListener(FETCH_SUPPORTED_REQUEST_EVENT, () => document.dispatchEvent(new CustomEvent(FETCH_SUPPORTED_RESPONSE_EVENT)));
    document.addEventListener(FETCH_REQUEST_EVENT, async event => {
      ...
      document.dispatchEvent(new CustomEvent(FETCH_RESPONSE_EVENT, {
        detail
      }));
    });
  ...
  if (globalThis.CSS && globalThis.CSS.paintWorklet && globalThis.CSS.paintWorklet.addModule) {
    ...
    globalThis.CSS.paintWorklet.addModule = function (moduleURL, options) {
      ...
      document.dispatchEvent(new CustomEvent(NEW_WORKLET_EVENT, {
        detail: {
          moduleURL,
          options
        }
  ...
  if (globalThis.FontFace) {
    ...
    globalThis.FontFace = function () {
      getDetailObject(...arguments).then(detail => document.dispatchEvent(new CustomEvent(NEW_FONT_FACE_EVENT, {
        detail
      })));
    ...
    document.fonts.delete = function (fontFace) {
      getDetailObject(fontFace.family).then(detail => document.dispatchEvent(new CustomEvent(DELETE_FONT_EVENT, {
        detail
      })));
  ...
  function getAdoptedStylesheetsListener(event) {
    ...
    if (shadowRoot) {
      ...
      shadowRoot.addEventListener(UNREGISTER_GET_ADOPTED_STYLESHEETS_REQUEST_EVENT, () => shadowRoot.removeEventListener(GET_ADOPTED_STYLESHEETS_REQUEST_EVENT, getAdoptedStylesheetsListener), {
        once: true
      });
      ...
      if (adoptedStyleSheets.length) {
        shadowRoot.dispatchEvent(new CustomEvent(GET_ADOPTED_STYLESHEETS_RESPONSE_EVENT, {
          detail: {
            adoptedStyleSheets
          }
        }));
