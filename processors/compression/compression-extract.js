...
async function extract(content, {
  ...
  async function getContent(resource) {
    return resource.filename.match(REGEXP_MATCH_FRAMES) && !resource.filename.match(REGEXP_MATCH_TOP_LEVEL_FRAME) || noBlobURL ? await getDataURI(resource.textContent, resource.mimeType) : URL.createObjectURL(new Blob([resource.textContent], {
      type: resource.mimeType
    }));
  }
  async function getDataURI(textContent, mimeType) {
    const reader = new FileReader();
    reader.readAsDataURL(new Blob([textContent], {
      type: mimeType
    }));
