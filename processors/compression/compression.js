...
var _zip = require("./../../vendor/zip/zip.js");
var _compressionExtract = require("./compression-extract.js");
var _compressionDisplay = require("./compression-display.js");
/*
...
const EXTRA_DATA_TAGS = [["<noscript>", "</noscript>"], ["<noframes>", "</noframes>"], ["<noembed>", "</noembed>"], ["<script type=sfz-data>", "</script>"], ["<style type=sfz-data>", "</style>"], ["<iframe>", "</iframe>"], ["<xmp>", "</xmp>"], ["<plaintext>", "</plaintext>"]];
const EMBEDDED_DATA_TAGS = [["<!--", "-->"], ...EXTRA_DATA_TAGS];
const EXTRA_DATA_REGEXPS = [[/<noscript/i, /<\/noscript>/i], [/<noframes/i, /<\/noframes>/i], [/<noembed/i, /<\/noembed>/i], [/<script/i, /<\/script>/i], [/<style/i, /<\/style>/i], [/<iframe/i, /<\/iframe>/i], [/<xmp/i, /<\/xmp>/i], [/<plaintext/i, /<\/plaintext>/i]];
const EMBEDDED_DATA_REGEXPS = [[/<!--/i, /-->/i], ...EXTRA_DATA_REGEXPS];
...
async function process(pageData, options, lastModDate = new Date()) {
  ...
  if (options.zipScript) {
    ...
    (0, _zip.configure)({
      workerScripts: {
        deflate: ["/lib/single-file-z-worker.js"]
      }
    });
    script = await (await fetch(browser.runtime.getURL(SCRIPT_PATH))).text();
  }
  const zipDataWriter = new _zip.Uint8ArrayWriter();
  zipDataWriter.init();
  ...
  const zipWriter = new _zip.ZipWriter(zipDataWriter, {
    bufferedWrite: true,
    keepOrder: false,
    lastModDate
  });
  ...
  }, options.createRootDirectory ? String(Date.now()) + "_" + (options.tabId || 0) + "/" : "", options.url);
  const data = await zipWriter.close(null, {
    preventClose: true
  });
  if (options.selfExtractingArchive) {
    ...
    if (options.extractDataFromPage) {
      const payload = await Promise.all([arrayToBase64(insertionsCRLF), arrayToBase64(substitutionsLF)]);
  ...
  if (options.embeddedImage) {
    ...
    return new Blob([pageContent, getCRC32(pageContent, embeddedImageDataOffset), new Uint8Array(options.embeddedImage.slice(options.embeddedImage.length - PNG_IEND_LENGTH))], {
      type: "application/octet-stream"
    });
  } else {
    return new Blob([pageContent], {
      type: "application/octet-stream"
    });
  }
}
...
async function prependHTMLData(pageData, zipDataWriter, script, options) {
  ...
  script = "<script>" + script + "document.currentScript.remove();" + "globalThis.addEventListener('load', () => {" + "globalThis.bootstrap=(()=>{let bootstrapStarted;return async content=>{if (bootstrapStarted) return bootstrapStarted; bootstrapStarted = (" + _compressionExtract.extract.toString().replace(/\n|\t/g, "") + ")(content,{prompt}).then(({docContent}) => " + _compressionDisplay.display.toString().replace(/\n|\t/g, "") + "(document,docContent," + JSON.stringify(displayOptions) + "));return bootstrapStarted;}})();(" + getContent.toString().replace(/\n|\t/g, "") + ")().then(globalThis.bootstrap).then(() => document.dispatchEvent(new CustomEvent(\"single-file-display-infobar\"))).catch(()=>{});" + "});" + "</script>";
  pageContent += script;
  let extraData = "";
...
async function arrayToBase64(data) {
  ...
  return await new Promise(resolve => {
    ...
    fileReader.readAsDataURL(new Blob([new Uint32Array(data)], {
      type: "application/octet-stream"
    }));
  });
}
...
async function addPageResources(zipWriter, pageData, options, prefixName, url) {
  ...
  await Promise.all([Promise.all([addFile(zipWriter, prefixName, {
    name: "index.html",
    extension: ".html",
    content: pageData.content,
    url,
    password: options.password
  }), addFile(zipWriter, prefixName, {
    name: "manifest.json",
    extension: ".json",
    content: jsonContent,
    password: options.password
  })]), Promise.all(Object.keys(pageData.resources).map(async resourceType => Promise.all(pageData.resources[resourceType].map(data => {
...
async function addFile(zipWriter, prefixName, data) {
  const dataReader = typeof data.content == "string" ? new _zip.TextReader(data.content) : new _zip.BlobReader(new Blob([new Uint8Array(data.content)]));
  const options = {
    comment: data.url && data.url.startsWith("data:") ? "data:" : data.url,
...
async function getContent() {
  ...
  const characterMap = new Map([[65533, 0], [8364, 128], [8218, 130], [402, 131], [8222, 132], [8230, 133], [8224, 134], [8225, 135], [710, 136], [8240, 137], [352, 138], [8249, 139], [338, 140], [381, 142], [8216, 145], [8217, 146], [8220, 147], [8221, 148], [8226, 149], [8211, 150], [8212, 151], [732, 152], [8482, 153], [353, 154], [8250, 155], [339, 156], [382, 158], [376, 159]]);
