...
exports.TIMEOUT_INIT_REQUEST_MESSAGE = void 0;
...
const TARGET_ORIGIN = "*";
const TIMEOUT_INIT_REQUEST_MESSAGE = exports.TIMEOUT_INIT_REQUEST_MESSAGE = 5000;
...
function init() {
  globalThis.addEventListener("message", async event => {
    if (typeof event.data == "string" && event.data.startsWith(_helper.MESSAGE_PREFIX)) {
      event.preventDefault();
      event.stopPropagation();
      const message = JSON.parse(event.data.substring(_helper.MESSAGE_PREFIX.length));
...
function initRequestSync(message) {
  ...
  if (!TOP_WINDOW) {
    ...
    sendInitResponse({
      frames: [getFrameData(document, globalThis, windowId, message.options, message.scrolling)],
      sessionId,
      requestedFrameId: document.documentElement.dataset.requestedFrameId && windowId
    });
    if (message.options.userScriptEnabled && waitForUserScript) {
...
function initResponse(message) {
  ...
  if (windowData) {
    ...
    message.frames.forEach(messageFrameData => {
      ...
      if (!frameData) {
        frameData = {
          windowId: messageFrameData.windowId
        };
...
function processFramesAsync(doc, frameElements, options, parentWindowId, sessionId) {
  ...
  sendInitResponse({
    frames,
    sessionId,
    requestedFrameId: doc.documentElement.dataset.requestedFrameId && parentWindowId
  });
  frameElements.forEach((frameElement, frameIndex) => {
    const windowId = parentWindowId + WINDOW_ID_SEPARATOR + frameIndex;
    try {
      sendMessage(frameElement.contentWindow, {
        ...
        sessionId,
        options,
        scrolling: frameElement.scrolling
      });
    } catch (error) {
      // ignored
    }
    requestTimeouts[windowId] = globalThis.setTimeout(() => sendInitResponse({
      frames: [{
        windowId,
        processed: true
      }],
...
function createFrameResponseTimeout(sessionId, windowId) {
  ...
  if (session && session.responseTimeouts) {
    session.responseTimeouts[windowId] = globalThis.setTimeout(() => sendInitResponse({
      frames: [{
        windowId: windowId,
        processed: true
      }],
...
function sendMessage(targetWindow, message, useChannel) {
  if (targetWindow == top && browser && browser.runtime && browser.runtime.sendMessage) {
    ...
    if (useChannel) {
      ...
      targetWindow.postMessage(_helper.MESSAGE_PREFIX + JSON.stringify({
        method: message.method,
        sessionId: message.sessionId
      }), TARGET_ORIGIN, [channel.port2]);
      channel.port1.postMessage(message);
    } else {
      targetWindow.postMessage(_helper.MESSAGE_PREFIX + JSON.stringify(message), TARGET_ORIGIN);
    }
  }
}
